 ...
 * query {@link ClusteringIndexFilter}. Some of the expressions of this filter
 * may be handled by a 2ndary index, and the rest is simply filtered out from
 * the result set (the later can only happen if the query was using ALLOW
 * FILTERING).
 */
public abstract class RowFilter implements Iterable<RowFilter.Expression> {
    ...
    /**
     * Filters the provided iterator so that only the row satisfying the expression
     * of this filter are included in the resulting iterator.
     *
     ...
     * @param nowInSec the current time in seconds (to know what is live and what
     *        isn't).
    ...
    /**
     * Returns true if all of the expressions within this filter that apply to the
     * partition key are satisfied by the given key, false otherwise.
     */
    ...
    /**
     * Returns true if all of the expressions within this filter that apply to the
     * clustering key are satisfied by the given Clustering, false otherwise.
     */
        ...
        public UnfilteredPartitionIterator filter(UnfilteredPartitionIterator iter, int nowInSec) {
            ...
            class IsSatisfiedFilter extends Transformation<UnfilteredRowIterator> {
                ...
                public UnfilteredRowIterator applyToPartition(UnfilteredRowIterator partition) {
                    ...
                    // Short-circuit all partitions that won't match based on static and partition
                    // keys
        ...
        public UnfilteredPartitionIterator filter(UnfilteredPartitionIterator iter, final int nowInSec) {
            ...
            class IsSatisfiedThriftFilter extends Transformation<UnfilteredRowIterator> {
                ...
                public UnfilteredRowIterator applyToPartition(UnfilteredRowIterator iter) {
                    // Thrift does not filter rows, it filters entire partition if any of the
                    // expression is not
                    // satisfied, which forces us to materialize the result (in theory we could
                    // materialize only
                    // what we need which might or might not be everything, but we keep it simple
                    // since in practice
                    // it's not worth that it has ever been).
                    ImmutableBTreePartition result = ImmutableBTreePartition.create(iter);
                    iter.close();

                    // The partition needs to have a row for every expression, and the expression
                    // needs to be valid.
                    ...
                    // If we get there, it means all expressions where satisfied, so return the
                    // original result
    ...
    public static abstract class Expression {
        ...
        protected enum Kind {
            SIMPLE, MAP_EQUALITY, THRIFT_DYN_EXPR, CUSTOM, USER
        }
         ...
         *         <code>IndexExpression</code> is a <code>CONTAINS</code> operator,
         ...
         *
         * @return <code>true</code> if the operator of this
         *         <code>IndexExpression</code> is a <code>CONTAINS_KEY</code> operator,
         *         <code>false</code> otherwise.
         */
        ...
        /**
         * If this expression is used to query an index, the value to use as partition
         * key for that index query.
         */
         ...
         * @param row the row to check. It should *not* contain deleted cells (i.e. it
         *        should come from a RowIterator).
        ...
        public boolean equals(Object o) {
            ...
            return Objects.equal(this.kind(), that.kind()) && Objects.equal(this.column.name, that.column.name)
                    && Objects.equal(this.operator, that.operator) && Objects.equal(this.value, that.value);
        }
        ...
        private static class Serializer {
            ...
            public Expression deserialize(DataInputPlus in, int version, CFMetaData metadata) throws IOException {
                ...
                if (version >= MessagingService.VERSION_30) {
                    ...
                    // custom expressions (3.0+ only) do not contain a column or operator, only a
                    // value
                    if (kind == Kind.CUSTOM) {
                        return new CustomExpression(metadata, IndexMetadata.serializer.deserialize(in, version, metadata),
                ...
                if (!metadata.isCompactTable() && column == null)
                    throw new RuntimeException(
                            "Unknown (or dropped) column " + UTF8Type.instance.getString(name) + " during deserialization");
    ...
    public static class SimpleExpression extends Expression {
        ...
        public void validate() {
            if (null == value) {
                checkTrue(Operator.EQ.equals(operator), "Unsupported operator %s for null value", operator.toString());
                checkFalse(column.isPrimaryKeyColumn(), "Unsupported null value for primary key column %s", column.name);
            } else {
                super.validate();
            }
        }
        ...
        public boolean isSatisfiedBy(CFMetaData metadata, DecoratedKey partitionKey, Row row) {
            ...
            case GT: {
                ...
                // In order to support operators on Counter types, their value has to be
                // extracted from internal
                ...
                if (column.type.isCounter()) {
                    ...
                    // Note that CQL expression are always of the form 'x < 4', i.e. the tested
                    // value is on the left.
                    ByteBuffer foundValue = getValue(metadata, partitionKey, row);
                    return (foundValue != null || Operator.EQ.equals(operator)) && operator.isSatisfiedBy(column.type, foundValue, value);
                }
            }
            case NEQ:
            case LIKE_PREFIX:
            ...
            case LIKE_MATCHES: {
                ...
                // Note that CQL expression are always of the form 'x < 4', i.e. the tested
                // value is on the left.
                return operator.isSatisfiedBy(column.type, foundValue, value);
            }
            case CONTAINS:
            ...
            case CONTAINS_KEY:
                ...
                if (column.isComplex()) {
                    ...
                    return foundValue != null
                            && mapType.getSerializer().getSerializedValue(foundValue, value, mapType.getKeysType()) != null;
                }

            case IN:
                // It wouldn't be terribly hard to support this (though doing so would imply
                // supporting
    ...
    /**
     * An expression of the form 'column' ['key'] = 'value' (which is only supported
     * when 'column' is a map).
     */
    private static class MapEqualityExpression extends Expression {
        ...
        public boolean equals(Object o) {
            ...
            return Objects.equal(this.column.name, that.column.name) && Objects.equal(this.operator, that.operator)
                    && Objects.equal(this.key, that.key) && Objects.equal(this.value, that.value);
        }
    ...
    private static class ThriftExpression extends Expression {
        ...
        private static ColumnDefinition makeDefinition(CFMetaData metadata, ByteBuffer name) {
            ...
            // In thrift, we actually allow expression on non-defined columns for the sake
            // of filtering. To accomodate
            // this we create a "fake" definition. This is messy but it works so is probably
            // good enough.
            return ColumnDefinition.regularDef(metadata, name, metadata.compactValueColumn().type);
        }

        public boolean isSatisfiedBy(CFMetaData metadata, DecoratedKey partitionKey, Row row) {
            // On thrift queries, even if the column expression is a "static" one, we'll
            // have convert it as a "dynamic"
            // one in ThriftResultsMerger, so we always expect it to be a dynamic one.
            // Further, we expect this is only
            // called when the row clustering does match the column (see ThriftFilter
            // above).
    ...
    /**
     * A custom index expression for use with 2i implementations which support
     * custom syntax and which are not necessarily linked to a single column in the
     * base table.
     */
    public static final class CustomExpression extends Expression {
        ...
        public CustomExpression(CFMetaData cfm, IndexMetadata targetIndex, ByteBuffer value) {
            // The operator is not relevant, but Expression requires it so for now we just
            // hardcode EQ
        ...
        private static ColumnDefinition makeDefinition(CFMetaData cfm, IndexMetadata index) {
            // Similarly to how we handle non-defined columns in thift, we create a fake
            // column definition to
            // represent the target index. This is definitely something that can be improved
            // though.
        ...
        public String toString() {
            return String.format("expr(%s, %s)", targetIndex.name,
                    Keyspace.openAndGetStore(cfm).indexManager.getIndex(targetIndex).customExpressionValueType());
        }
     ...
     * programmatically by a QueryHandler implementation. No concrete
     * implementations are provided and adding custom impls to the classpath is a
     * task for operators (needless to say, this is something of a power user
     * feature). Care must also be taken to register implementations, via the static
     * register method during system startup. An implementation and its
     * corresponding Deserializer must be registered before sending or receiving any
     * messages containing expressions of that type. Use of custom filtering
     * expressions in a mixed version cluster should be handled with caution as the
     * order in which types are registered is significant: if continuity of use
     * during upgrades is important, new types should registered last and obsoleted
     * types should still be registered ( or dummy implementations registered in
     * their place) to preserve consistent identifiers across the cluster).
     *
     * During serialization, the identifier for the Deserializer implementation is
     * prepended to the implementation specific payload. To deserialize, the
     * identifier is read first to obtain the Deserializer, which then provides the
     * concrete expression instance.
     */
    public static abstract class UserExpression extends Expression {
        ...
        private static final class DeserializerRegistry {
            ...
            public void registerUserExpressionClass(Class<? extends UserExpression> expressionClass,
                ...
                logger.debug("Registered user defined expression type {} and serializer {} with identifier {}", expressionClass.getName(),
        ...
        protected static abstract class Deserializer {
            protected abstract UserExpression deserialize(DataInputPlus in, int version, CFMetaData metadata) throws IOException;
        }
        ...
        private static long serializedSize(UserExpression expression, int version) { // 4 bytes for the expression type id
